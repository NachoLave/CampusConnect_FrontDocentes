...
/**
 * Proxy API para los endpoints de disponibilidad del módulo docente
...
export async function GET(request: Request) {
  try {
    ...
    console.error('Error in availability proxy (GET):', error)
...
export async function POST(request: Request) {
  try {
    ...
    console.error('Error in availability proxy (POST):', error)
    return NextResponse.json({ error: error?.message || 'Proxy error' }, { status: 500 })
  }
}
